...
namespace SkillFactory_Module_12
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.Write("Enter your login: ");  // Ввод логина
            string userLogin = Console.ReadLine();
            bool member = false;

            for (int i = 0; i < listOfUsers.Length; i++)   //Проверяем логин, тип подписки
            {
                if (listOfUsers[i].Login == userLogin)
                {
                    Console.WriteLine($"Hello, {listOfUsers[i].Name}!");
                    member = true;
                    if (listOfUsers[i].IsPremium == false)  // Показ рекламы
                        ShowAds();
                    break;
                }
             }

            if (!member)                       //Если нет никакой подписки
            {
